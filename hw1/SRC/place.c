	 ...
	 int *x_lookup)
{
	...
	{
	    bb_coord_new = (struct s_bb *)my_malloc(2 * max_pins_per_fb * //opt
						    sizeof(struct s_bb));
	    bb_edge_new = (struct s_bb *)my_malloc(2 * max_pins_per_fb *
						   sizeof(struct s_bb));
	    nets_to_update =
		(int *)my_malloc(2 * max_pins_per_fb * sizeof(int)); //opt
	    net_block_moved =
    ...
    //opt grid[x_to][y_to] is used frequently but never aliasing
    //no subroutine that might bring side effect in the function. 
    z_to = 0;
    ...
    for(k = 0; k < num_nets_affected; k++)
	{
	  ...
	  int ynew;
          int inet2= inet;
	  struct s_bb* bb_coord_new2= &bb_coord_new[bb_index];;
	  struct s_bb* bb_edge_new2= &bb_edge_new[bb_index];
...
//				  y_from);
                     ...
                     /////////////////////////////////////////////////////////////// update bb start//////////////////////
   {
     ...
     * the bb_coord_new2 data structure and the number of blocks on each   *
     * edge in the bb_edge_new2 data structure.  This routine should only  *
	...
	{			/* Move to left. */
	    ...
	    if(xold == bb_coords[inet2].xmax)
		{		/* Old position at xmax. */
		    if(bb_num_on_edges[inet2].xmax == 1)
			{
			    get_bb_from_scratch(inet2, bb_coord_new2,
						bb_edge_new2);
//			    return;
			}
		    else
			{
			    bb_edge_new2->xmax =
				bb_num_on_edges[inet2].xmax - 1;
			    bb_coord_new2->xmax = bb_coords[inet2].xmax;
			}
		}
		...
		{		/* Move to left, old postion was not at xmax. */
		    bb_coord_new2->xmax = bb_coords[inet2].xmax;
		    bb_edge_new2->xmax = bb_num_on_edges[inet2].xmax;
		}

	    /* Now do the xmin fields for coordinates and number of edges. */

	    if(xnew < bb_coords[inet2].xmin)
		{		/* Moved past xmin */
		    bb_coord_new2->xmin = xnew;
		    bb_edge_new2->xmin = 1;
		}

	    else if(xnew == bb_coords[inet2].xmin)
		{		/* Moved to xmin */
		    bb_coord_new2->xmin = xnew;
		    bb_edge_new2->xmin = bb_num_on_edges[inet2].xmin + 1;
		}

	    else
		{		/* Xmin unchanged. */
		    bb_coord_new2->xmin = bb_coords[inet2].xmin;
		    bb_edge_new2->xmin = bb_num_on_edges[inet2].xmin;
		}
	}
	...
	{			/* Move to right. */
	    ...
	    if(xold == bb_coords[inet2].xmin)
		{		/* Old position at xmin. */
		    if(bb_num_on_edges[inet2].xmin == 1)
			{
			    get_bb_from_scratch(inet, bb_coord_new2,
						bb_edge_new2);
//			    return;
			}
		    else
	    ...
	    if(xnew > bb_coords[inet2].xmax)
		{		/* Moved past xmax. */
		    bb_coord_new2->xmax = xnew;
		    bb_edge_new2->xmax = 1;
		}

	    else if(xnew == bb_coords[inet2].xmax)
		{		/* Moved to xmax */
		    bb_coord_new2->xmax = xnew;
		    bb_edge_new2->xmax = bb_num_on_edges[inet2].xmax + 1;
		}
	...
	{			/* Move down. */
	    ...
	    if(yold == bb_coords[inet2].ymax)
		{		/* Old position at ymax. */
		    if(bb_num_on_edges[inet2].ymax == 1)
			{
			    get_bb_from_scratch(inet, bb_coord_new2,
						bb_edge_new2);
//			    return;
			...
			{
			    bb_edge_new2->ymax =
				bb_num_on_edges[inet2].ymax - 1;
			    bb_coord_new2->ymax = bb_coords[inet2].ymax;
			}
		}
		...
		{		/* Move down, old postion was not at ymax. */
		    bb_coord_new2->ymax = bb_coords[inet2].ymax;
		    bb_edge_new2->ymax = bb_num_on_edges[inet2].ymax;
		}

	    /* Now do the ymin fields for coordinates and number of edges. */

	    if(ynew < bb_coords[inet2].ymin)
		{		/* Moved past ymin */
		    bb_coord_new2->ymin = ynew;
		    bb_edge_new2->ymin = 1;
		}

	    else if(ynew == bb_coords[inet2].ymin)
		{		/* Moved to ymin */
		    bb_coord_new2->ymin = ynew;
		    bb_edge_new2->ymin = bb_num_on_edges[inet2].ymin + 1;
		}

	    else
		{		/* ymin unchanged. */
		    bb_coord_new2->ymin = bb_coords[inet2].ymin;
		    bb_edge_new2->ymin = bb_num_on_edges[inet2].ymin;
		}
	}
	...
	{			/* Moved up. */
	    ...
	    if(yold == bb_coords[inet2].ymin)
		{		/* Old position at ymin. */
		    if(bb_num_on_edges[inet2].ymin == 1)
			{
			    get_bb_from_scratch(inet2, bb_coord_new2,
						bb_edge_new2);
//			    return;
	    ...
	    if(ynew > bb_coords[inet2].ymax)
		{		/* Moved past ymax. */
		    bb_coord_new2->ymax = ynew;
		    bb_edge_new2->ymax = 1;
		}

	    else if(ynew == bb_coords[inet2].ymax)
		{		/* Moved to ymax */
		    bb_coord_new2->ymax = ynew;
		    bb_edge_new2->ymax = bb_num_on_edges[inet2].ymax + 1;
		}
       ...
       place_algorithm == PATH_TIMING_DRIVEN_PLACE)
	{
	    ...
	    comp_delta_td_cost(b_from, b_to, num_of_pins, &timing_delta_c,
			       &delay_delta_c); //opt subroutine by taking a look inside
    ...
    if(keep_switch)
	{
	    ...
	    *bb_cost = *bb_cost + bb_delta_c;
	    ...
	    grid[x_from][y_from].blocks[z_from] = b_to; //opt grid[from]???
	...
	{			/* Move was rejected.  */
	    ...
	    block[b_from].x = x_from; //opt block ?
		   ...
		   int num_regions)
{
    ...
    for(i = 0; i < num_regions; i++)
	{
        struct s_place_region * s = place_region_x[i];
         struct s_place_region * y =place_region_y[i];
	    for(j = 0; j < num_regions; j++)
		{
		    s[j].occupancy = old_region_occ_x[i][j];
                   // assert(place_region_y[i][j].occupancy == y[j].occupancy);
		   ...
		   float *delta_delay)
{
    ...
    for(k = 0; k < num_of_pins; k++)
	{
	    inet = block[b_from].nets[k];
            ...
            struct s_net net_inet =net[inet];
            
	    if(inet == OPEN)
		continue;

	    if(net_inet.is_global)
		continue;
    ...
    source_block = net[inet2].node_block[0];
    source_type = block[source_block].type;

    sink_block = net[inet2].node_block[ipin2];
    sink_type = block[sink_block].type;
    ...
    if(delay_source_to_sink < 0.)
	{
		...
		{		/*this net is driving a moved block               */
		    ...
		    if(net_inet.node_block[0] != b_to
		       && net_inet.node_block[0] != b_from)
			{
			    temp_delay =delay_source_to_sink;
//				comp_td_point_to_point_delay(inet, net_pin);
		...
		{		/*this net is being driven by a moved block, recompute */
		    /*all point to point connections on this net. */
                int num_sinks =net_inet.num_sinks; //opt by avoid mem access
...
//								     net_pin);
			...
			{	/*this net is being driven by a moved block, recompute */
			    /*all point to point connections on this net. */
                        int num_sinks =net[inet].num_sinks;
			    for(ipin = 1; ipin <= num_sinks;ipin++)
				{
	      ...
	      float *connection_delay_sum)
{
    ...
    if(delay_source_to_sink < 0.)
	{
	    ...
	    exit(1);
                                    ...
                                    //////////////////////////////////////////////
			    temp_delay_cost =delay_source_to_sink;
				//comp_td_point_to_point_delay(inet, ipin);
			    temp_timing_cost =
				temp_delay_cost *
				timing_place_crit[inet][ipin];
		      ...
		      struct s_bb *bb_coord_new)
{
    ...
    //opt by avoid repeatitive calc
    int num_sinks_one = (net[inet].num_sinks + 1);
        for(k = 1; k < num_sinks_one; k++)
	{
