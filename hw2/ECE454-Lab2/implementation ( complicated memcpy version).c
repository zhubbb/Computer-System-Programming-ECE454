...
#define mirror_x(i,j,w,i2,j2) i2=i;j2=w-j-1
#define mirror_y(i,j,w,i2,j2) i2=w-i-1;j2=j
...
void processAll(unsigned char* rendered_frame,struct info* info_array,int obj_size , unsigned width, unsigned height,
...
//        int index=0;
        ...
        int height_partition = height / 100;
        int big_row_width= height_partition*width;
        int big_row_size = big_row_width*3;
        
        char* white_buffer = malloc(big_row_size);// = (char *)malloc(row_size);
         memset(white_buffer,white,big_row_size);
        
        int small_width= width/30;
        ...
        for(int i=0;i< height; i+=height_partition  ){
            
            
            
            
        int bound_i=  i+height_partition;
        bound_i=bound_i>height? height:bound_i;
       
//          
        if(memcmp(&f[i*width],white_buffer,(bound_i-i)*width*3)){ /////////////////////////////////////////////////////////////////important here
//            if(memcmp(&f[index],white_buffer,row_size)){ // check if the whole role is white
                
            
          for(int i2=i;i2<bound_i;++i2){ //need to deal with leftover i
                
             int index=i2*width;
            
         //       printf("%d\n",rs);
            int iter = width / small_width;
            
            for(int si=0; si<iter; ++si){
...
//            int current_row_index=small_width*iter;
//            index+=current_row_index;
//            
//            if( leftover&&memcmp(&f[index],small_white_buffer,leftover)){
//                
