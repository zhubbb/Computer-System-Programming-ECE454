...
void processAll(unsigned char* rendered_frame,struct info* info_array,int obj_size , int* obj_width_array,int* obj_row_index_array ,int obj_height,
...
//    unsigned char* rendered_frame = allocateFrame(width, height);
    ...
    //tiling
   //assume 64 byte cacheline , widht = 21 , height = 64  block
...
//    printf("%d w,%d h",width,height);
    ...
    for(int i=0; i< obj_height ; i=i+block_size){
        int row_window = (obj_height -i);
        row_window = row_window < block_size? row_window : block_size;
        int row_terminate=0;
        
    for(int j=0; row_window> row_terminate ; j+=block_size){ //control when to stop column index ++ for this blocking area
        for(int i2 =i ; i2 < block_size+i2 && i2 < obj_height; ++i2){
        for(int j2=j; j2<block_size+j2 && j2< obj_width_array[i2]; ++j2 ){
            int row = obj_row_index_array[i2];
            struct info inf = info_array[row+j2];
...
//        int index=0;
        ...
        int* obj_row_index_array = (int *)malloc(height*sizeof(int));
        ...
        int* info_width_array= (int *)malloc(height*sizeof(int));
        int info_height=0;
...
//                    ++index;
    ...
    //                    printf("a:%d,b:%d,c:%d\n",c.a,c.b,c.c);
            ...
            if(info_row_width){
//                printf("info row width : %d\n",info_row_width);
//                printf("obj_row_index : %d\n",obj_row_index);
//                printf("height : %d\n",info_height);
                
                info_width_array[info_height]= info_row_width;
                
                obj_row_index_array[info_height]=obj_row_index;
                
                ++info_height;
                
                
                obj_row_index+=info_row_width;
...
//           }//the first if bracket
        ...
        info_width_array=realloc(info_width_array,info_height*sizeof(int));
        obj_row_index_array=realloc(obj_row_index_array,info_height*sizeof(int));;
//        printf("info size : %d\n",info_index);
...
//            printf("%d:y\n",mirror_y);
                 ...
                 memset(f,white,size);  
                processAll(frame_buffer,info_array,info_index, info_width_array,obj_row_index_array , info_height , width,height,shifthorizontal,shiftvertical,rotate,mirror_x,mirror_y);
//                 memset(r,white,size);
