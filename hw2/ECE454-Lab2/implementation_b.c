...
//        int index=0;
	 ...
	 char tiny_white_buffer[block_size*3];
        memset(tiny_white_buffer,0xff,block_size*3);
         ...
         char* white_buffer = malloc(row_size);
        memset(white_buffer,white,row_size);
        ...
        for(int i=0;i< height; i=i+block_size  ){
            ...
            for(int j=0; j< width; j= j+block_size){
                ...
                tiling_boundj= tiling_boundj>width? width: tiling_boundj;
                
                
                for(int i2=i;i2< tiling_boundi; ++i2){
                    int index=i2*width;
                    int window_size = tiling_boundj-j;
                    window_size=window_size<block_size? window_size:block_size;
                    if(memcmp(&f[index+j],tiny_white_buffer,window_size)){
...
//            
//         //       printf("%d\n",rs);
//             int tiling_index_end_i = i+ block_size;
//             tiling_index_end_i= tiling_index_end_i>height? height: tiling_index_end_i;
//             
//            
...
//                   
//                    for(int j2=offset;j2< j2_end; j2=j2+block_size){
//                          
//                         //calculate tiling index
//                          int tiling_index_end_j = ((j2/block_size)+1)*block_size;
//                          tiling_index_end_j = tiling_index_end_j > j2_end? j2_end: tiling_index_end_j;
//                          
//                          
//                          for(int i2=i; i2 < tiling_index_end_i; ++i2){
//                            for(int j3= j2; j3< tiling_index_end_j; ++j3){
//
//
//                              c = f[index+j2];
//                              if( (char)(c.a^0xff) || (char)(c.b^0xff) || (char)(c.c^0xff) ){
