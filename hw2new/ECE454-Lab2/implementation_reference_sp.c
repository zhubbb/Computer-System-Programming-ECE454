...
void pre_processing_ref(struct info* info_array_modified,struct info* info_array,int obj_size , unsigned width, unsigned height,
...
void processAll_ref( struct _three_byte_* r,struct info** info_array_array,int obj_size , unsigned width, unsigned height,
                               int x, int y,int rotate_iteration,int mirror_x, int mirror_y) {
//    printf("here1\n");
//
//        printf("here1");
...
//        info_array_array[6]= info_array_my;
     for(int i =0 ; i < obj_size; ++i){
        ...
        int h=inf.x+x;
//         printf("%d v,%d h",v,h);
        r[assign_index(v,h,width)]=inf.color;
...
//           }//the first if bracket
        ...
        struct info* info_array_r90 = malloc(info_index * sizeof(struct info));
        struct info* info_array_r180 =malloc(info_index * sizeof(struct info));
        struct info* info_array_r270 = malloc(info_index * sizeof(struct info));
        struct info* info_array_r90_mx = malloc(info_index * sizeof(struct info));
        struct info* info_array_r90_my= malloc(info_index * sizeof(struct info));
        struct info* info_array_mx = malloc(info_index * sizeof(struct info));
        struct info* info_array_my = malloc(info_index * sizeof(struct info));
        
        pre_processing_ref(info_array_r90,info_array,info_index,width,height,0,0,1,0,0);
        pre_processing_ref(info_array_r180,info_array,info_index,width,height,0,0,2,0,0);
        pre_processing_ref(info_array_r270,info_array,info_index,width,height,0,0,3,0,0);
        pre_processing_ref(info_array_r90_mx,info_array,info_index,width,height,0,0,1,1,0);
        pre_processing_ref(info_array_r90_my,info_array,info_index,width,height,0,0,1,0,1);
        pre_processing_ref(info_array_mx,info_array,info_index,width,height,0,0,0,1,0);
        pre_processing_ref(info_array_my,info_array,info_index,width,height,0,0,0,0,1);
...
//             printf("%d:test\n",info_array_array[0][0].color);
             ...
             if(rotate&&mirror_x){
                ...
                post_shifth = shifthorizontal;
                 ...
                 memset(f,white,size);  
                processAll_ref(f,info_array_array,info_index,width,height,post_shifth,post_shiftv,rotate,mirror_x,mirror_y);
//                 memset(r,white,size);
