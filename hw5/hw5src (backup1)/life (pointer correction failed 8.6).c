...
void init(int nrows, int ncols, char* info_buffer, char* inboard );
void  update_hori_boundary(int nrows,int ncols, char* buffer1, char* buffer2 );
void  update_verti_boundary(int nrows,int ncols, char* buffer1, char* buffer2 );
...
//    char info_buffer2[new_rows][new_cols];
    ...
    char info_buffer[newsize];
    char info_buffer2[newsize];
...
#define BOARD( __board, __i, __j )  (__board[(__i)*ncols + (__j)])   
    ...
    for (int i = 0; i < nrows; i++)
    {
        ...
        for (int j = 0; j < ncols; j++)
        {
            ...
            info_buffer[il+j] = neighbor_count;
...
#define UPDATE( ilj,inorth,isouth ,jwest, jeast,buffer2,new_state) \
    int change = new_state? 1:-1;\
    buffer2[jwest] += change;\
    buffer2[jeast] += change;\
    buffer2[inorth] += change;\
    buffer2[isouth] += change;\
    buffer2[inorth+1] += change;\
    buffer2[isouth+1] += change;\
    buffer2[inorth-1] += change;\
    buffer2[isouth-1] += change;\
    buffer2[ilj] = buffer2[ilj] + (change<<4);\
...
//    const int LDA = nrows;
    ...
    char *buffer1= info_buffer;
    char *buffer2= info_buffer2;
...
//                    printf("update %d\n",update);
                     ...
                     int inorth = ilj-ncols; //mod (i-1, nrows); // for i = 1  , i==0 seperately loop to avoid bounding issue , separte thread? no sepearte thread just first check and another loop if found
                     int isouth = ilj+ncols;//mod (i+1, nrows);
                     int jwest = ilj-1;//mod (j-1, ncols);
                     int jeast = ilj+1;//mod (j+1, ncols);

                    if(update){
                       int new_state = new_status.state;
    //                     record[i][j]=update;
                       UPDATE(ilj,inorth,isouth,jwest,jeast,buffer2,new_state);
...
void  update_hori_boundary(int nrows,int ncols, char* buffer1, char* buffer2 ){
        for(int j = 0 ;j <ncols; j++){
            ...
            int inorth = (nrows-1)*ncols+j;//mod (i-1, nrows); 
            int isouth = ilj+ncols;//mod (i+1, nrows);
            int jwest =  il+LOWBOUND (j-1, ncols);
            int jeast =  il+HIGHBOUND (j+1, ncols);
            if(update){
            ...
            il = i*ncols;
            ilj=il+j;
            info_key= buffer1[ilj];
            new_status =  status_map[info_key]; //(char)((neighbour_count|state)==3); 
            update = new_status.update;
            inorth = ilj-ncols;//nrows-2;//mod (i-1, nrows); 
            isouth = j;//mod (i+1, nrows);
            jwest =  il+LOWBOUND (j-1, ncols);
            jeast =  il+HIGHBOUND (j+1, ncols);
            if(update){
                char new_state =  new_status.state;
                UPDATE(ilj,inorth,isouth,jwest,jeast,buffer2,new_state);
            }
...
void  update_verti_boundary(int nrows,int ncols, char* buffer1, char* buffer2 ){
    for(int i = 1 ;i <nrows-1; i++){
        ...
        int inorth = ilj-ncols;//mod (i-1, nrows);
        int isouth = ilj+ncols;//mod (i+1, nrows);
        int jwest =  il+ncols-1;//mod (j-1, ncols);
        int jeast =  il+1;//mod (j+1, ncols);
        ...
        inorth = ilj-ncols;//mod (i-1, nrows);
        isouth = ilj+ncols;//mod (i+1, nrows);
        jwest =  il+ncols-2; //mod (j-1, ncols);
        jeast =  il+0;//mod (j+1, ncols);
