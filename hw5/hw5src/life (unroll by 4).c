...
//        lockg;
       ...
       //  printf("resource beore:%d\n",resource);
       // BARRIER;
       // printf("resource after:%d\n",resource);
        pthread_barrier_wait(barrier);
...
//            if(1){update_verti_boundary_i_j0
                ...
                for (j = 1; j < ncols-4; j=j+4)
                {
                    ...
                    int info_key3= buffer1[i][j+2];
                    status new_status3 =  status_map[info_key3];
                    int update3= new_status3.update;
                    if(update3){
                       int jwest = j+1;//mod (j-1, ncols);
                       int jeast = j+3;//mod (j+1, ncols);
                       int new_state3 = new_status3.state;
                       UPDATE(i,(j+2),inorth,isouth,jwest,jeast,buffer2,new_state3);
                    }
                    int info_key4= buffer1[i][j+3];
                    status new_status4 =  status_map[info_key4];
                    int update4= new_status4.update;
                    if(update4){
                       int jwest = j+2;//mod (j-1, ncols);
                       int jeast = j+4;//mod (j+1, ncols);
                       int new_state4 = new_status4.state;
                       UPDATE(i,(j+3),inorth,isouth,jwest,jeast,buffer2,new_state4);
                    }
